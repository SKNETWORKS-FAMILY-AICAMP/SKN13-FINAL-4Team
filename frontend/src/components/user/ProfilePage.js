...
function ProfilePage() {
    ...
    // [추가] .env 파일에서 API 기본 URL을 가져옵니다 (이미지 경로용).
    const apiBaseUrl = process.env.REACT_APP_API_BASE_URL || '';

    useEffect(() => {
        const fetchUserData = async () => {
            try {
                // [수정] api 모듈을 사용하여 요청합니다. 헤더는 자동으로 추가됩니다.
                ...
                // api 모듈의 인터셉터가 토큰 만료 시 로그인 페이지로 보내므로,
                // 여기서는 일반적인 에러 처리만 합니다.
    ...
    const handleImageUpload = async (imageFile) => {
        ...
        try {
            // [수정] api 모듈을 사용하고, 헤더를 직접 설정합니다 (파일 업로드 시 필요).
