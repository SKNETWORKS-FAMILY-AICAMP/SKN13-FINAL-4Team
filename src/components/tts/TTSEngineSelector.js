...
const AITTSEngineSelector = ({ 
  ...
  const checkEngineStatus = async () => {
    ...
    try {
      ...
      // 전용 상태 확인 API 사용
      console.log(`📡 TTS 상태 확인 API 호출: ${baseUrl}/api/chat/ai/tts/status/`);
  ...
  const getCurrentVoiceOptions = () => {
    return ELEVENLABS_VOICE_OPTIONS; // ElevenLabs 전용
  };
  ...
  const getCurrentVoiceKey = () => {
    return 'elevenLabsVoice'; // ElevenLabs 전용
  };
